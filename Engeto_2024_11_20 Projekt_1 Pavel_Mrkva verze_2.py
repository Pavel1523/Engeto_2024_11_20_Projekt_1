# projekt_1.py: první projekt do Engeto Online Python Akademie, verze 2, oproti verzi 1 oprava kontroly user a password - zamezení akceptace nesprávné dvojice a oprava histogramu = adaptivní přizpůsobení šířky pole s hvězdičkami množství hvězdiček
    ...
    if password[index] == heslo:
        ...
        for attempt in range(3):  # 3 pokusy na zadani spravne vety
            ...
            if veta_i in {"1", "2", "3"}:  # Kontrola, zda je vstup správný
                ...
                print(
                    f"{"LEN":>5}|{"OCCURENCES":^{int(nejdelsi_slovo) + 8}}|{"NR.":<4}"
                )
                print((int(nejdelsi_slovo) + 5 + 5 + 8 + 2) * "-")
